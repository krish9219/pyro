...
void cmd_doc(const std::string& path) {
    std::string source = read_source(path);
    std::string base = fs::path(path).stem().string();
    std::string output = base + "_doc.html";

    // Parse source for functions and comments
    std::vector<std::pair<std::string, std::string>> docs; // {signature, doc_comment}
    std::istringstream ss(source);
    std::string line, last_comment;
    while (std::getline(ss, line)) {
        // Trim
        ...
        if (trimmed.size() > 1 && trimmed[0] == '#' && trimmed[1] == ' ') {
            if (!last_comment.empty()) last_comment += "\n";
            last_comment += trimmed.substr(2);
        } else if (trimmed.find("fn ") == 0) {
            // Function definition
            std::string sig = trimmed;
            docs.push_back({sig, last_comment});
            last_comment.clear();
    ...
    std::vector<std::string> imports;
    std::istringstream ss2(source);
    while (std::getline(ss2, line)) {
        std::string t = line;
        while (!t.empty() && t[0] == ' ') t.erase(0, 1);
        if (t.find("import ") == 0) {
            imports.push_back(t.substr(7));
        }
    }
    ...
    f << "<title>" << base << " — Pyro Documentation</title>\n";
    f << "<style>\n";
    f << "body{font-family:-apple-system,sans-serif;max-width:800px;margin:40px auto;padding:0 20px;background:#0a0a1a;color:#e0e0e0}\n";
    f << "h1{color:#ff6b35;border-bottom:2px solid #1a1a2e;padding-bottom:12px}\n";
    f << "h2{color:#ff8c42;margin-top:32px}\n";
    f << "h3{color:#4ade80;margin-top:24px}\n";
    f << ".fn{background:#1a1a2e;padding:16px;border-radius:8px;margin:12px 0;border-left:3px solid #ff6b35}\n";
    f << ".fn code{color:#ff8c42;font-size:1.05em}\n";
    f << ".fn p{color:#94a3b8;margin:8px 0 0;font-size:0.9em}\n";
    f << ".imports{background:#1a1a2e;padding:12px 16px;border-radius:8px}\n";
    f << ".imports span{display:inline-block;background:#252540;padding:4px 10px;border-radius:4px;margin:3px;color:#4ade80;font-family:monospace;font-size:0.9em}\n";
    f << ".meta{color:#64748b;font-size:0.85em;margin-bottom:24px}\n";
    f << "</style>\n</head>\n<body>\n";
    f << "<h1>" << base << ".ro</h1>\n";
    f << "<p class='meta'>Generated by <code>pyro doc</code> — Pyro Programming Language</p>\n";

    // Imports
    if (!imports.empty()) {
        f << "<h2>Imports</h2>\n<div class='imports'>\n";
        for (auto& imp : imports) f << "<span>import " << imp << "</span>\n";
        f << "</div>\n";
    }

    // Functions
    if (!docs.empty()) {
        f << "<h2>Functions (" << docs.size() << ")</h2>\n";
        for (auto& [sig, doc] : docs) {
            f << "<div class='fn'>\n<code>" << sig << "</code>\n";
            if (!doc.empty()) f << "<p>" << doc << "</p>\n";
            f << "</div>\n";
        }
    }

    f << "<hr style='border-color:#1a1a2e;margin-top:40px'>\n";
    f << "<p style='color:#64748b;font-size:0.8em;text-align:center'>Built with Pyro — P.Y.R.O — Performance You Really Own</p>\n";
    f << "</body>\n</html>\n";
    f.close();

    std::cout << "Documentation generated: " << output << std::endl;
    std::cout << "  " << docs.size() << " functions documented\n";
    std::cout << "  " << imports.size() << " imports found\n";
}
...
void cmd_repl() {
    ...
    while (true) {
        ...
        // REPL commands
        if (trimmed == ":quit" || trimmed == ":q" || trimmed == ":exit" || trimmed == "exit" || trimmed == "quit") break;

        if (trimmed == ":help" || trimmed == ":h") {
            std::cout << "REPL Commands:\n";
            std::cout << "  :help    Show this help\n";
            std::cout << "  :clear   Clear accumulated code\n";
            std::cout << "  :quit    Exit REPL\n";
            std::cout << "  :time    Toggle timing display\n";
            std::cout << "\nType Pyro code and press Enter to evaluate.\n";
            std::cout << "Multi-line blocks (fn, if, for, while, struct, match, try)\n";
            std::cout << "are read until a blank line.\n";
            continue;
        }
        ...
        if (trimmed == ":time" || trimmed == ":t") {
            show_timing = !show_timing;
            std::cout << "Timing: " << (show_timing ? "on" : "off") << "\n";
            continue;
        }
